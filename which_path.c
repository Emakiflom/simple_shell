...
{
    unsigned int com_length, pa_length, orig_pa_length;  /* Length variables */
    ...
    if (path_copy == NULL)
    {
        errors(3);  /* Print an error message and return NULL on memory allocation failure */
        return (NULL);
    }
    ...
    while (token != NULL)
    {
        ...
        /* Allocate memory for the full path of the command within the current directory */
        ...
        /* Construct the full path by concatenating the directory path and the command */
        str_cpy(fullpath, token);
        fullpath[pa_length] = '/';
        str_cpy(fullpath + pa_length + 1, command);
        fullpath[pa_length + com_length + 1] = '\0';

        /* Check if the full path is executable (has execute permissions) */
        if (access(fullpath, X_OK) != 0)
        {
            free(fullpath);  /* Free memory if the path is not executable */
            fullpath = NULL;
            token = strtok(NULL, ":");  /* Move to the next directory in the PATH */
        }
        else
            break;  /* Exit the loop if the executable path is found */
    }
