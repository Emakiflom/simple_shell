...
/**
 * main - The program enters a continuous loop that represents
           the main shell execution.
 * Prompt and User Input: prompt(STDIN_FILENO, buf): Displays
           the shell prompt if in interactive mode.
 * line = get_line(stdin): Reads a line of input from the user.
 * Check for Empty Input: Checks if the entered line is empty
           (contains only a newline character).If so, skips to the next iteration.
 * Tokenization: tokens = tokenizer(line): Tokenizes the input
               line into individual commands and arguments. 
 * Execute Built-in Commands: builtin_status = buiilt_in_execu(tokens): Checks and
         executes built-in commands such as "exit" or "env."
 * If a built-in command was successful or the user requested exit, free allocated
         memory and continue to the next iteration.
...
int main(void)
{
    ...
    while (TRUE)
    {
        ...
        /* If the first token is NULL, meaning an empty line, skip to the next iteration */
        if (tokens[0] == NULL)
        {
            free(tokens);
            continue;
        ...
        /* Execute built-in commands (e.g., exit) or continue to external command execution */
        builtin_status = buiilt_in_execu(tokens);

        /* Free memory allocated for tokens and input line if necessary */
        ...
        /* If the built-in command was successful or the user requested exit, continue to the next iteration */
        if (builtin_status == 0)
            continue;
        ...
        if (fullpath == NULL)
            ...
            flag = 1; /* Set the flag to 1 to indicate that fullpath was dynamically allocated */
        ...
        /* Free allocated memory for tokens, the PATH variable, the input line, and fullpath if it was dynamically allocated */
