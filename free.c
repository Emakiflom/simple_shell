...
/**
 * free_all - This function is designed to free dynamically allocated memory at
              the end of the main loop in your shell program.
...
/**
 * free_dp - This function is responsible for freeing a double pointer (array) and the memory
              occupied by the strings it points to.
 * The function uses a for loop to iterate through each element of the double pointer.
 * It uses the free function to release the memory occupied by the strings
              pointed to by the double pointer.
 * Finally, it frees the memory occupied by the double pointer itself using free(array).
...
void free_dp(char **array, unsigned int length)
{
    ...
    for (i = 0; i < length; i++)
    {
        /* Free the memory occupied by the strings pointed to by the double pointer */
        free(array[i]);
    }
