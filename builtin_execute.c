 ...
 * It iterates through an array of built-in commands and their corresponding functions.
...
int buiilt_in_execu(char **tokens)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (str_cmp(tokens[0], builtin[i].name, length) == 0)
        {
            ...
            /* Return the status, indicating the success or failure of the built-in function */
            return (status);
        }
    }
...
int shell_no_builtins(built_s builtin[])
{
    ...
    /* Iterate through the array until a NULL entry is encountered, counting the number of built-in commands */
